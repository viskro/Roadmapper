...
try {
    ...
    $user_id = checkUserAuth("isFinished");
    ...
    // Format attendu: /api/isFinished.php/123 où 123 est l'ID de l'item
    ...
    if (!$item) {
        sendJsonResponse(false, "Item non trouvé ou vous n'êtes pas autorisé à le y toucher", [], 404);
    }
    ...
    $result = $itemModel->isFinished($id);
    
    if ($result) {
        // Si la suppression a réussi, envoyer une réponse de succès
        sendJsonResponse(true, "Item récupéré avec succès", ["id" => $id]);
    } else {
        // Si la suppression a échoué pour une raison inconnue
        sendJsonResponse(false, "Échec de la récupération de l'item pour une raison inconnue", [], 500);
    }
