...
try {
    ...
    $user_id = checkUserAuth("setIsFinished");
    ...
    // Format attendu: /api/setIsFinished.php/123 où 123 est l'ID de l'item
    ...
    if (!$item) {
        sendJsonResponse(false, "Item non trouvé ou vous n'êtes pas autorisé à le modifier", [], 404);
    }
    
    // Modifier l'item en utilisant la méthode du modèle
    // Cette méthode vérifie déjà que l'utilisateur est propriétaire de l'item
    $result = $itemModel->setIsFinished($id);
